...
export default function Dashboard() {
  ...
  const stakings =[
    ...
    {name: 'Lido Finance', chain:'https://etherscan.io/token/0xf610a9dfb7c89644979b4a0f27063e9e7d7cda32', TVL:'$10,929,659,170', app_link:'https://lido.fi/'}
  ]
          ...
          {compound.length > 0 && <>
            ...
            <div
              ...
              {compound.map((item, index) => (<>
                    ...
                    <span className="text-center px-4 py-2">{parseFloat(item.quote_rate > 0 ? item.quote_rate : item.debt_quote_rate).toFixed(4)}</span>
                    <span className="text-center px-4 py-2">{parseFloat(item.quote > 0 ? item.quote : item.debt_quote).toFixed(4)}</span>
                  </>
              ))}
            </div>
          </>}
